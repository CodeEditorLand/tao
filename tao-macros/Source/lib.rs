...
/// Generates a JNI binding for a Rust function so you can use it as the extern
/// for Java/Kotlin class methods in your android app.
///
/// This macro expects 5 mandatory parameters and 3 optional:
/// 1. snake_case representation of the reversed domain of the app. for example:
///    com_tao
/// 2. snake_case representation of the package name. for example: tao_app
/// 3. Java/Kotlin class name.
/// 4. Rust function name (`ident`).
/// 5. List of extra types your Rust function expects. Pass empty array if the
///    function doesn't need any arugments.
///   - If your function takes an arguments as reference with a lifetime tied to
///     the [`JNIEnv`], it should use `'local` as the lifetime name as it is the
///     lifetime name generated by the macro. Note that all rust functions
///     should expect the first two parameters to be [`JNIEnv`] and [`JClass`]
///     so make sure they are imported into scope).
/// 6. (Optional) Return type of your rust function.
///   - If your function returns a reference with a lifetime tied to the
///     [`JNIEnv`], it should use `'local` as the lifetime name as it is the
///     lifetime name generated by the macro.
///   - if you want to use the next macro parameter you need to provide a type
///     or just pass `__VOID__` if the function doesn't return anything.
/// 7. (Optional) List of `ident`s to pass to the rust function when invoked
///    (This mostly exists for internal usage by `tao` crate).
/// 8. (Optional) Function to be invoked right before invoking the rust function
///    (This mostly exists for internal usage by `tao` crate).
///
/// ## Example 1: Basic
///
...
/// unsafe fn add(_env:JNIEnv, _class:JClass, a:i32, b:i32) -> i32 { a + b }
/// ```
...
/// # type JClass<'a> = JNIEnv<'a>;
/// #[no_mangle]
/// unsafe extern fn Java_com_example_tao_OperationsClass_add<'local>(
/// 	env:JNIEnv<'local>,
/// 	class:JClass<'local>,
/// 	a_1:i32,
/// 	a_2:i32,
...
/// Generate the package name used for invoking Java/Kotlin methods at
/// compile-time
///
...
/// To escape the `_` character you can use `_1` which aligns with the default
/// NDK implementation.
///
/// ## Example
///
...
pub fn generate_package_name(tokens:TokenStream) -> TokenStream {
	...
	// note that this character is invalid in an identifier so it's safe to use
	// as replacement
