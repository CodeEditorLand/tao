...
pub struct ProgressBarState {
	...
	/// The `.desktop` filename with the Unity desktop window manager, for
	/// example `myapp.desktop` **Linux Only**
...
impl WindowBuilder {
	...
	/// ## Platform-specific:
	///
	/// - **Windows**: It is recommended to always use the same theme used in
	///   [`EventLoopBuilderExtWindows::with_theme`] for this method also or use
	///   `None` so it automatically uses the theme used in
	///   [`EventLoopBuilderExtWindows::with_theme`] or falls back to the system
	///   preference, because [`EventLoopBuilderExtWindows::with_theme`] changes
	///   the theme for some controls like context menus which is app-wide and
	///   can't be changed by this method.
	///
...
impl Window {
	...
	#[inline]
	pub fn title(&self) -> String { self.window.title() }
	...
	/// - **iOS / Android:** Unsupported.
	#[inline]
	pub fn set_focus(&self) { self.window.set_focus() }
	...
	/// - **iOS / Android:** Unsupported.
	#[inline]
	pub fn is_focused(&self) -> bool { self.window.is_focused() }
	...
	/// - **iOS / Android:** Unsupported.
	#[inline]
	pub fn is_always_on_top(&self) -> bool { self.window.is_always_on_top() }
	...
	/// ## Platform-specific
	///
	/// - **Linux:** Most size methods like maximized are async and do not work
	///   well with calling
	/// sequentailly. For setting inner or outer size, you don't need to set
	/// resizable to true before it. It can resize no matter what. But if you
	/// insist to do so, it has a `100, 100` minimum limitation somehow. For
	/// maximizing, it requires resizable is true. If you really want to set
	/// resizable to false after it. You might need a mechanism to check the
	...
	/// - **iOS / Android:** Unsupported.
	#[inline]
	pub fn is_minimized(&self) -> bool { self.window.is_minimized() }
	...
	/// - **Linux / iOS / Android:** Unsupported.
	#[inline]
	pub fn is_minimizable(&self) -> bool { self.window.is_minimizable() }
	...
	/// - **Linux / iOS / Android:** Unsupported.
	#[inline]
	pub fn is_maximizable(&self) -> bool { self.window.is_maximizable() }
	...
	/// - **iOS / Android:** Unsupported.
	#[inline]
	pub fn is_closable(&self) -> bool { self.window.is_closable() }
	...
	///
	/// - **iOS / Android:** Unsupported.
	pub fn is_decorated(&self) -> bool { self.window.is_decorated() }
	...
	/// ## Platform-specific
	///
	/// - **Windows**: There is no guarantee that the window will be the bottom
	///   most but it will try to be.
	/// - **iOS / Android:** Unsupported.
	#[inline]
	...
	/// ## Platform-specific
	///
	/// - **Linux / macOS**: Unlike windows, progress bar is app-wide and not
	///   specific to this window. Only supported desktop environments with
	///   `libunity` (e.g. GNOME).
	/// - **iOS / Android:** Unsupported.
	#[inline]
	...
	/// - **iOS / Android:** Unsupported.
	#[inline]
	pub fn theme(&self) -> Theme { self.window.theme() }
...
impl Window {
	...
	///
	/// If `true`, the events are passed through the window such that any other
	/// window behind it receives them. If `false` the window will catch the
	/// cursor events. By default cursor events are not ignored.
	///
	/// ## Platform-specific
	///
	/// - **iOS / Android:** Always returns an [`ExternalError::NotSupported`]
...
impl rwh_06::HasWindowHandle for Window {
	fn window_handle(&self) -> Result<rwh_06::WindowHandle<'_>, rwh_06::HandleError> {
		...
		// SAFETY: The window handle will never be deallocated while the window is
		// alive.
...
pub struct WindowSizeConstraints {
	/// The minimum width a window can be, If this is `None`, the window will
	/// have no minimum width (aside from reserved).
	///
	/// The default is `None`.
	pub min_width:Option<PixelUnit>,
	/// The minimum height a window can be, If this is `None`, the window will
	/// have no minimum height (aside from reserved).
	///
	/// The default is `None`.
	pub min_height:Option<PixelUnit>,
	/// The maximum width a window can be, If this is `None`, the window will
	/// have no maximum width (aside from reserved).
	///
	/// The default is `None`.
	pub max_width:Option<PixelUnit>,
	/// The maximum height a window can be, If this is `None`, the window will
	/// have no maximum height (aside from reserved).
	///
	/// The default is `None`.
