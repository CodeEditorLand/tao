...
pub(crate) struct Layout {
	...
	/// We're not using `ModifiersState` here because that object cannot
	/// express caps lock, but we need to handle caps lock too.
	///
	...
	/// wouldn't change the keyboard state (it clears the dead key). There is
	/// a flag to prevent changing the state, but that flag requires Windows
	/// 10, version 1607 or newer)
...
impl Layout {
	pub fn get_key(
		...
		if !unknown_alt {
			// Here we try using the virtual key directly but if the virtual key
			// doesn't distinguish between left and right alt, we can't
			// report AltGr. Therefore, we only do this if the key is not the
			// "unknown alt" key.
			//
			// The reason for using the virtual key directly is that
			// `MapVirtualKeyExW` (used when building the keys map) sometimes
			// maps virtual keys to odd scancodes that don't match
...
impl LayoutCache {
	...
	fn prepare_layout(
		...
		// `MapVirtualKeyExW` maps (non-numpad-specific) virtual keys to
		// scancodes as if numlock was off. We rely on this behavior to find
		// all virtual keys which are not numpad-specific but map to the
		// numpad.
		//
		// src_vkey: VK  ==>  scancode: u16 (on the numpad)
		//
		// Then we convert the source virtual key into a `Key` and the scancode
		// into a virtual key to get the reverse mapping.
		//
		// src_vkey: VK  ==>  scancode: u16 (on the numpad)
		//    ||                    ||
		//    \/                    \/
		...
		for mod_state in 0..mods_end {
			...
			// This is reinforced by the fact that the keyboard state array has
			// 256 elements. This array is allowed to be indexed by virtual
		...
		// Second pass: replace right alt keys with AltGr if the layout has alt
		// graph
...
fn vkey_to_non_char_key(
	...
	// List of the Web key names and their corresponding platform-native key
	// names: https://developer.mozilla.org/en-US/docs/Web/API/KeyboardEvent/key/Key_Values
	...
	match vkey {
		...
		// win32km::VK_HANGEUL => Key::HangulMode, // Deprecated in favour of
		// VK_HANGUL
		...
		// win32km::VK_OEM_FJ_JISHO => Key::Unidentified(native_code), //
		// Conflicts with `VK_OEM_NEC_EQUAL`
