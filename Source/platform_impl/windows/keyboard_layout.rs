...
impl Layout {
	pub fn get_key(
		...
		if !unknown_alt {
			...
			// distinguish between left and right alt, we can't report AltGr. Therefore,
			// we only do this if the key is not the "unknown alt" key.
			//
			...
			// scancodes that don't match the scancode coming from the KEYDOWN message
			// for the same key. For example: `VK_LEFT` is mapped to `0x004B`, but the
			// scancode for the left arrow is `0xE04B`.
			let key_from_vkey =
...
impl LayoutCache {
	...
	fn prepare_layout(strings:&mut HashSet<&'static str>, locale_id:HKL) -> Layout {
		...
		// numlock was off. We rely on this behavior to find all virtual keys which
		// are not numpad-specific but map to the numpad.
		//
...
fn vkey_to_non_char_key(
	...
	match vkey {
		...
		// win32km::VK_OEM_FJ_JISHO => Key::Unidentified(native_code), // Conflicts with
		// `VK_OEM_NEC_EQUAL`
