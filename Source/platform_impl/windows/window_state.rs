...
use windows::Win32::{
	...
	Graphics::Gdi::{HRGN, InvalidateRgn},
...
impl WindowState {
	...
	pub fn has_active_focus(&self) -> bool { self.is_active && self.is_focused }
...
impl CursorFlags {
	fn refresh_os_cursor(self, window:HWND) -> Result<(), io::Error> {
		...
		if util::is_focused(window) {
			...
			// event loop with `WM_MOUSEMOVE` events, and `refresh_os_cursor` is called
			// by `set_cursor_flags` which at times gets called once every iteration of
			// the eventloop.
