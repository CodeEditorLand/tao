...
use windows::Win32::{
	Foundation::HWND,
	Graphics::Gdi::{HRGN, RDW_INTERNALPAINT, RedrawWindow},
};
...
impl<T> EventLoopRunner<T> {
	...
	pub fn wait_thread_id(&self) -> u32 { self.wait_thread_id }
...
impl<T> EventLoopRunner<T> {
	...
	unsafe fn call_new_events(&self, init:bool) {
		...
		let _ = RedrawWindow(self.thread_msg_target, None, HRGN::default(), RDW_INTERNALPAINT);
	}
