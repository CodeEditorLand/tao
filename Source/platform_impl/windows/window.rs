...
impl Window {
	...
	pub fn set_overlay_icon(&self, icon:Option<&Icon>) {
		...
		unsafe {
			taskbar.SetOverlayIcon(self.window.0, icon, None).unwrap_or(());
		}
	}
...
unsafe fn init<T:'static>(
	...
	let real_window = {
		...
		let (target_monitor, position) = attributes
			.position
			.and_then(|p| {
				monitor::available_monitors().into_iter().find_map(|monitor| {
		...
		let adjusted_size = {
			let (w, h):(i32, i32) =
				clamped_size.to_physical::<u32>(target_monitor.scale_factor()).into();
...
unsafe fn register_window_class(window_classname:&str) -> Vec<u16> {
	...
	// CreateWindowEx anyway. Also since there is no weird element in the struct,
	// there is no reason for this  call to fail.
...
unsafe extern "system" fn window_proc(
	...
	// attached. we need this because we need to respond to WM_NCCALCSIZE as soon
	// as possible in order to make the window borderless if needed.
	match msg {
...
unsafe fn force_window_active(handle:HWND) {
	...
	// permission. We only call this function in the window creation, so it should
	// be fine. See: https://stackoverflow.com/questions/10740346/setforegroundwindow-only-working-while-visual-studio-is-open
