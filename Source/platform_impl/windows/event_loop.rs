...
fn main_thread_id() -> u32 {
	...
	static INIT_MAIN_THREAD_ID:unsafe fn() = {
		unsafe fn initer() { MAIN_THREAD_ID = GetCurrentThreadId(); }
		initer
	};
...
fn create_event_target_window() -> HWND {
	...
	util::SetWindowLongPtrW(
		...
		// messages (which are used for delivering events during resizes),
		// but it isn't displayed to the user because of the LAYERED style.
...
///
/// Tao's API guarantees that all redraw events will be clustered together and
...
/// events. This function explicitly pulls all of Tao's redraw events out of the
/// event queue so that they always all get processed in one fell swoop.
///
	...
	let callback = || {
		match msg {
			...
			win32wm::WM_DPICHANGED => {
				...
				// a correct size for the client area.
				if !is_decorated {
					style &= !WS_CAPTION;
					style &= !WS_SIZEBOX;
				}
				...
				// When the "Show window contents while dragging" is turned off,
				// there is no need to adjust the window size.
...
unsafe fn handle_raw_input<T:'static>(
	...
	if data.header.dwType == RIM_TYPEMOUSE.0 {
		...
		if scancode == 0xE11D || scancode == 0xE02A {
			// At the hardware (or driver?) level, pressing the Pause key is
			// equivalent to pressing Ctrl+NumLock.
			// This equvalence means that if the user presses Pause, the
			// keyboard will emit two subsequent keypresses:
			// 1, 0xE11D - Which is a left Ctrl (0x1D) with an extension flag
			// (0xE100) 2, 0x0045 - Which on its own can be interpreted as
			// Pause
			//
			// There's another combination which isn't quite an equivalence:
			// PrtSc used to be Shift+Asterisk. This means that on some
			// keyboards, presssing PrtSc (print screen) produces the
			// following sequence: 1, 0xE02A - Which is a left shift (0x2A)
			// with an extension flag (0xE000) 2, 0xE037 - Which is a numpad
			// multiply (0x37) with an exteion flag (0xE000). This on
			//             its own it can be interpreted as PrtSc
			//
			// For this reason, if we encounter the first keypress, we simply
			// ignore it, trusting that there's going to be another event
			// coming, from which we can extract the appropriate key.
		...
		let code = if VIRTUAL_KEY(keyboard.VKey) == VK_NUMLOCK {
			// Historically, the NumLock and the Pause key were one and the same
			// physical key. The user could trigger Pause by pressing
			// Ctrl+NumLock. Now these are often physically separate and the
			// two keys can be differentiated by checking the extension flag
			// of the scancode. NumLock is 0xE045, Pause is 0x0045.
			//
			// However in this event, both keys are reported as 0x0045 even on
			// modern hardware. Therefore we use the virtual key instead to
			// determine whether it's a NumLock and set the KeyCode
			// accordingly.
			//
			// For more on this, read the article by Raymond Chen, titled:
			// "Why does Ctrl+ScrollLock cancel dialogs?"
		...
		if VIRTUAL_KEY(keyboard.VKey) == VK_SHIFT {
			match code {
				...
				| KeyCode::Numpad8
				| KeyCode::Numpad9 => {
					// On Windows, holding the Shift key makes numpad keys
					// behave as if NumLock wasn't active. The way this is
					// exposed to applications by the system is that
					// the application receives a fake key release event for the
					// shift key at the moment when the numpad key is
					// pressed, just before receiving the numpad key
					// as well.
					//
					// The issue is that in the raw device event (here), the
					// fake shift release event reports the numpad key as
					// the scancode. Unfortunately, the event doesn't
					// have any information to tell whether it's the left shift
					// or the right shift that needs to get the fake
					// release (or press) event so we don't forward this
					// event to the application at all.
					//
					// For more on this, read the article by Raymond Chen,
					// titled: "The shift key overrides NumLock"
