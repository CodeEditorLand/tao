...
unsafe fn public_window_callback_inner<T:'static>(
	...
	let callback = || {
		match msg {
				...
				if let (
					...
					*GET_POINTER_DEVICE_RECTS,
					...
					for pointer_info in pointer_infos.iter().rev() {
						...
						// between the resolution of the display device (pixel) and the touch
						// device (himetric).
			...
			// Only sent on Windows 8.1 or newer. On Windows 7 and older user has to log out to
			// change DPI, therefore all applications are closed while DPI is changing.
			win32wm::WM_DPICHANGED => {
				...
				// that "you only need to use either the X-axis or the Y-axis value when
				// scaling your application since they are the same".
				...
				// `AdjustWindowRect*` functions doesn't account for the hidden caption and
				// borders and calculates a correct size for the client area.
				if !is_decorated {
					style &= !WS_CAPTION;
				...
				// However, win32 doesn't provide an `UnadjustWindowRectEx` function to get
				// the client rect from the outer rect, so we instead adjust the window rect
				// to get the decoration margins and remove them from the outer size.
				let margin_left:i32;
				let margin_top:i32;
				...
				// size after exiting fullscreen (the restored size is already DPI
				// adjusted).
				let mut new_physical_inner_size = match allow_resize {
					// We calculate our own size because the default suggested rect doesn't do a
					// great job of preserving the window's logical size.
					true => {
				...
				// When the "Show window contents while dragging" is turned off, there is no
				// need to adjust the window size.
				...
				let new_outer_rect:RECT;
				{
					...
					new_outer_rect = {
						if conservative_rect_monitor != new_dpi_monitor {
							...
							// The direction to nudge the window in to get the window onto the
							// monitor with the new DPI factor. We calculate this by seeing
							// which monitor edges are shared and nudging away from the wrong
							// monitor based on those.
...
unsafe extern "system" fn thread_event_target_callback<T:'static>(
	...
	let callback = || {
		match msg {
			...
			// Because WM_PAINT comes after all other messages, we use it during modal loops to
			// detect when the event queue has been emptied. See `process_event` for more
			// details.
			win32wm::WM_PAINT => {
				let _ = ValidateRect(window, None);
				// If the WM_PAINT handler in `public_window_callback` has already flushed the
				// redraw events, `handling_events` will return false and we won't emit a
				// second `RedrawEventsCleared` event.
				if subclass_input.event_loop_runner.handling_events() {
					if subclass_input.event_loop_runner.should_buffer() {
						...
						// This WM_PAINT handler will never be re-entrant because
						// `flush_paint_messages` doesn't call WM_PAINT for the thread event
						// target (i.e. this window).
					...
					subclass_input.event_loop_runner.control_flow()
				{
					...
					while Instant::now() < wait_until {
						if PeekMessageW(&mut msg, HWND::default(), 0, 0, PM_NOREMOVE).as_bool() {
							...
							// RDW_INTERNALPAINT set (i.e. doesn't have an update region),
							// PeekMessageW will remove that window from the redraw queue
							// even though we told it not to remove messages from the
		...
		if VIRTUAL_KEY(keyboard.VKey) == VK_SHIFT {
			match code {
				...
				| KeyCode::Numpad9 => {
					...
					// doesn't have any information to tell whether it's the left shift or the
					// right shift that needs to get the fake release (or press) event so we
					// don't forward this event to the application at all.
					//
					// For more on this, read the article by Raymond Chen, titled:
