...
use windows::{
	Win32::{
		...
		Graphics::Gdi::{ClientToScreen, HMONITOR, HRGN, InvalidateRgn},
...
pub(crate) fn set_inner_size_physical(window:HWND, x:i32, y:i32, is_decorated:bool) {
	unsafe {
		let rect =
			adjust_window_rect(window, RECT { top:0, left:0, bottom:y, right:x }, is_decorated)
...
#[allow(non_snake_case)]
#[inline]
pub fn HIWORD(dword:u32) -> u16 { ((dword & 0xFFFF_0000) >> 16) as u16 }
...
#[allow(non_snake_case)]
#[inline]
pub fn PRIMARYLANGID(hkl:HKL) -> u32 { ((hkl.0 as usize) & 0x3FF) as u32 }
