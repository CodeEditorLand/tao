...
pub(crate) fn set_inner_size_physical(
	...
	unsafe {
		let rect = adjust_window_rect(
			window,
			RECT { top:0, left:0, bottom:y as i32, right:x as i32 },
			is_decorated,
		)
...
pub fn adjust_window_rect(
	...
	unsafe {
		...
		// so `AdjustWindowRect*` functions doesn't account for the hidden
		// caption and borders and calculates a correct size for the client
		// area.
		if !is_decorated {
			style &= !WS_CAPTION;
			style &= !WS_SIZEBOX;
