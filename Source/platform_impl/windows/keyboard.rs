...
///
/// A single Tao `KeyEvent` contains information which the Windows API passes to
/// the application in multiple window messages. In other words: a Tao
/// `KeyEvent` cannot be built from a single window message. Therefore, this
/// type keeps track of certain information from previous events so
/// that a `KeyEvent` can be constructed when the last event related to a
...
impl KeyEventBuilder {
	...
	pub(crate) fn process_message(
		...
		match msg_kind {
			win32wm::WM_SETFOCUS => {
				...
				let key_events = self
					.synthesize_kbd_state(ElementState::Pressed, &kbd_state);
			...
			win32wm::WM_KILLFOCUS => {
				...
				let key_events = self
					.synthesize_kbd_state(ElementState::Released, &kbd_state);
				if !key_events.is_empty() {
					return key_events;
				}
			},
			...
			win32wm::WM_DEADCHAR | win32wm::WM_SYSDEADCHAR => {
				*result = ProcResult::Value(LRESULT(0));
				// At this point, we know that there isn't going to be any more
				// events related to this key press
			...
			win32wm::WM_CHAR | win32wm::WM_SYSCHAR => {
				if self.event_info.is_none() {
					trace!(
						"Received a CHAR message but no `event_info` was \
						 available. The message is probably IME, returning."
					);
					return vec![];
				}
				*result = ProcResult::Value(LRESULT(0));
				...
				if is_utf16 {
					...
					// Let's encode it as UTF-16 and append it to the end of
					// `utf16parts`
					let utf16parts = match self.event_info.as_mut() {
						Some(ev_info) => &mut ev_info.utf16parts,
						None => {
							warn!(
								"The event_info was None when it was expected \
								 to be some"
							);
							return vec![];
						},
				...
				if !more_char_coming {
					...
					let mut layouts = LAYOUT_CACHE.lock();
					// It's okay to call `ToUnicode` here, because at this point
					// the dead key is already consumed by the character.
					let kbd_state = get_kbd_state();
					let mod_state =
					...
					// modifiers because that already consumed the dead key.
					// Otherwise, we would interpret the character
					// incorrectly, missing the dead key.
					if !ctrl_on {
						event_info.text =
						...
						let num_lock_on =
							kbd_state[usize::from(VK_NUMLOCK.0)] & 1 != 0;
	...
	fn synthesize_kbd_state(
		...
		let is_key_pressed =
			|vk:VIRTUAL_KEY| &kbd_state[usize::from(vk.0)] & 0x80 != 0;
		...
		// We are synthesizing the press event for caps-lock first for the
		// following reasons:
		...
		//    keypress after every other key, in which case all other keys
		//    *must* be sythesized as if the caps-lock state was be the opposite
	...
	fn create_synthetic(
		...
		let scancode = unsafe {
			MapVirtualKeyExW(u32::from(vk.0), MAPVK_VK_TO_VSC_EX, locale_id)
		};
		if scancode == 0 {
			return None;
		}
		...
		let key_without_modifiers = layout.get_key(
			WindowsModifiers::empty(),
			false,
			vk,
			scancode,
...
enum PartialLogicalKey {
	...
	/// combined with the second input, and in that case we should fall back to
	/// using what would have without a dead-key input.
	TextOr(Key<'static>),
...
impl PartialKeyEventInfo {
	fn from_message(
		...
		let scancode = if lparam_struct.scancode == 0 {
			// In some cases (often with media keys) the device reports a
			// scancode of 0 but a valid virtual key. In these cases we
			// obtain the scancode from the virtual key.
			unsafe {
				MapVirtualKeyExW(
					u32::from(vkey.0),
					MAPVK_VK_TO_VSC_EX,
		...
		// On Windows Ctrl+NumLock = Pause (and apparently Ctrl+Pause ->
		// NumLock). In these cases the KeyCode still stores the real key, so
		// in the name of consistency across platforms, we circumvent this
		// mapping and force the key values to match the keycode. For more on
		// this, read the article by Raymond Chen, titled:
		...
		let preliminary_logical_key = layout.get_key(
			mods_without_ctrl,
			num_lock_on,
			vkey,
			scancode,
		...
		{
			// In some cases we want to use the UNICHAR text for logical_key in
			// order to allow dead keys to have an effect on the character
			// reported by `logical_key`.
		...
		let key_without_modifiers = if let Some(key) = code_as_key {
			...
			match layout.get_key(NO_MODS, false, vkey, scancode, code) {
				...
				// The reason for this is that `key_without_modifiers` is
				// designed for key-bindings, but the US International
				// layout treats `'` (apostrophe) as a dead key and the
				// reguar US layout treats it a character. In order for a single
				// binding configuration to work with both layouts, we
				// forward each dead key as a character.
				Key::Dead(k) => {
					if let Some(ch) = k {
						// I'm avoiding the heap allocation. I don't want to
						// talk about it :(
						let mut utf8 = [0; 4];
						let s = ch.encode_utf8(&mut utf8);
						let static_str =
							get_or_insert_str(&mut layouts.strings, s);
	...
	fn finalize(self, strings:&mut HashSet<&'static str>) -> KeyEvent {
		...
		let logical_key = match self.logical_key {
			PartialLogicalKey::TextOr(fallback) => {
				match text {
					Some(s) => {
					...
					None => {
						Key::Unidentified(NativeKeyCode::Windows(self.scancode))
					},
				}
			},
...
fn is_current_fake(
	...
	let curr_is_ctrl =
		matches!(curr_info.logical_key, PartialLogicalKey::This(Key::Control));
...
fn get_location(scancode:ExScancode, hkl:HKL) -> KeyLocation {
	...
	match vkey {
		...
		| win32km::VK_LCONTROL
		| win32km::VK_LMENU
		| win32km::VK_LWIN => KeyLocation::Left,
		win32km::VK_RSHIFT
		| win32km::VK_RCONTROL
		| win32km::VK_RMENU
		| win32km::VK_RWIN => KeyLocation::Right,
