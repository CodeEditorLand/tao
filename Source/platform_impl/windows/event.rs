...
fn layout_uses_altgr() -> bool {
	unsafe {
		...
		// AltGr is an alias for Ctrl+Alt for... some reason. Whatever it is, those are
		// the keypresses we have to emulate to do an AltGr test.
	...
	let vkey = match vkey {
		...
		_ => {
			match scancode {
				...
				// sequence, but reports VK_PAUSE as the virtual key on both events, or VK_PAUSE
				// on the first event or 0xFF when using raw input. Don't emit anything for the
				// LeftControl event in the pair...
				...
				// reports a different scancode when used with modifiers than when used without
				0xE046 => {
					scancode = 0xE059;
					VK_PAUSE
				},
