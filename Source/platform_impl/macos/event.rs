...
pub fn code_to_key(code:KeyCode, scancode:u16) -> Key<'static> {
	match code {
		...
		// Other numpad keys all generate text on macOS (if I understand
		// correctly)
...
pub fn get_scancode(event:cocoa::base::id) -> c_ushort {
	...
	// layout-dependent character. In tao, we use keycode to refer to the key's
	// character, and so this function aligns AppKit's terminology with ours.
