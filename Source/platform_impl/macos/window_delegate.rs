...
pub struct WindowDelegateState {
	...
	// We keep this ns_view because we still need its view state for some
	// extern function like didResignKey
...
impl WindowDelegateState {
	...
	fn ns_view(&self) -> id { unsafe { (*self.ns_window).contentView() } }
...
lazy_static! {
	static ref WINDOW_DELEGATE_CLASS: WindowDelegateClass = unsafe {
		...
		decl.add_method(
			...
			mark_is_checking_zoomed_in as extern fn(&Object, Sel),
		);
		decl.add_method(
			sel!(clearIsCheckingZoomedIn),
			clear_is_checking_zoomed_in as extern fn(&Object, Sel),
		);
		...
		decl.add_method(
			...
			effective_appearance_did_change as extern fn(&Object, Sel, id),
		);
		decl.add_method(
			sel!(effectiveAppearanceDidChangedOnMainThread:),
			effective_appearance_did_changed_on_main_thread as extern fn(&Object, Sel, id),
		);
...
extern fn mark_is_checking_zoomed_in(this:&Object, _sel:Sel) {
	with_state(&*this, |state| {
		state.is_checking_zoomed_in = true;
	});
}

extern fn clear_is_checking_zoomed_in(this:&Object, _sel:Sel) {
	with_state(&*this, |state| {
...
extern fn effective_appearance_did_change(this:&Object, _:Sel, _:id) {
...
extern fn effective_appearance_did_changed_on_main_thread(this:&Object, _:Sel, _:id) {
	with_state(this, |state| {
