	...
	unsafe {
		...
		let frame = match screen {
			...
			None => {
				...
				let desired_size =
					attrs.inner_size.unwrap_or_else(|| PhysicalSize::new(800, 600).into());
	...
	unsafe {
		...
		match event_type {
			appkit::NSLeftMouseDown => {
				...
				// `WindowBuilder::with_movable_by_window_background` is not working.
				// Because of this, we need to invoke `[NSWindow performWindowDragWithEvent]` in
...
impl UnownedWindow {
	pub fn new(
		...
		unsafe {
			...
			if win_attribs.inner_size_constraints.has_min() {
				let min_size = win_attribs.inner_size_constraints.min_size_logical(scale_factor);

				set_min_inner_size(*ns_window, min_size);
			}
			if win_attribs.inner_size_constraints.has_max() {
				let max_size = win_attribs.inner_size_constraints.max_size_logical(scale_factor);
	...
	pub fn set_background_color(&self, color:Option<crate::window::RGBA>) {
		unsafe {
			let color = color
				...
				.unwrap_or_else(|| if self.transparent { NSColor::clearColor(nil) } else { nil });
	...
	#[inline]
	pub fn is_decorated(&self) -> bool { self.decorations.load(Ordering::Acquire) }
	...
	pub fn set_badge_label(&self, label:Option<String>) { set_badge_label(label); }
}
...
impl WindowExtMacOS for UnownedWindow {
	...
	#[inline]
	fn ns_view(&self) -> *mut c_void { unsafe { (*self.ns_window).contentView() as *mut _ } }
	...
	fn set_badge_label(&self, label:Option<String>) { set_badge_label(label); }
}
