...
pub fn new_view(ns_window:id) -> (IdRef, Weak<Mutex<CursorState>>) {
	...
	unsafe {
		...
		(IdRef::new(msg_send![ns_view, initWithTao: state_ptr]), cursor_access)
	}
}
...
lazy_static! {
	static ref VIEW_CLASS: ViewClass = unsafe {
		...
		decl.add_method(
			sel!(initWithTao:),
			init_with_tao as extern fn(&Object, Sel, *mut c_void) -> id,
		);
		decl.add_method(
...
extern fn init_with_tao(this:&Object, _sel:Sel, state:*mut c_void) -> id {
	unsafe {
...
extern fn unmark_text(this:&mut Object, _sel:Sel) {
...
extern fn do_command_by_selector(_this:&Object, _sel:Sel, _command:Sel) {
	...
	// TODO: (Artur) all these inputs seem to trigger a key event with the
	// correct text content so this is not needed anymore, it seems.
...
extern fn key_down(this:&mut Object, _sel:Sel, event:id) {
	...
	unsafe {
		...
		if pass_along {
			// The `interpretKeyEvents` above, may invoke `set_marked_text` or
			// `insert_text`, if the event corresponds to an IME event.
			// If `set_marked_text` or `insert_text` were not invoked, then the
			// IME was deactivated, and we should cancel the IME session.
			// When using arrow keys in an IME window, the input context won't
			// invoke the IME related methods, so in that case we shouldn't
			// cancel the IME session.
...
extern fn flags_changed(this:&Object, _sel:Sel, ns_event:id) {
	use KeyCode::{
		AltLeft,
		AltRight,
		ControlLeft,
		...
		macro_rules! process_event {
			($tao_flag:expr, $ns_flag:expr, $target_key:expr) => {
				...
				if actual_key == KeyCode::KeyA {
					// When switching keyboard layout using Ctrl+Space, the Ctrl
					// release event has `KeyA` as its keycode which would
					// produce an incorrect key event. To avoid this, we
					// detect this scenario and override the key with one that
					...
					if matching_keys && is_pressed && was_pressed {
						// If we received a modifier flags changed event AND the
						// key that triggered it was already pressed then
						// this must mean that this event indicates the
						// release of that key.
						is_pressed = false;
					}
				}
