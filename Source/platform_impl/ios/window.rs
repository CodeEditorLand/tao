...
impl Inner {
	...
	pub fn request_redraw(&self) {
		unsafe {
			if self.gl_or_metal_backed {
				...
				// CAEAGLLayer or CAMetalLayer. Ordinarily the OS sets up a bunch of UIKit
				// state before calling drawRect: on a UIView, but when using
	...
	pub fn set_badge_count(&self, count:i32) { set_badge_count(count); }
}
