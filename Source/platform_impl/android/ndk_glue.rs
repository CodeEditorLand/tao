...
///
/// Arguments in order:
/// 1. android app domain name in reverse snake_case as an ident (for ex:
///    com_example)
/// 2. android package anme (for ex: wryapp)
/// 3. the android activity that has external linking for the following
...
/// 4. a one time setup function that will be ran once after tao has created its
///    event loop in the `create` function above.
...
pub unsafe fn create(
	...
	// Don't return from this function (`ANativeActivity_onCreate`) until the
	// thread has created its `ThreadLooper` and assigned it to the static
...
pub unsafe fn start(_:JNIEnv, _:JClass, _:JObject) { wake(Event::Start); }

pub unsafe fn stop(_:JNIEnv, _:JClass, _:JObject) { wake(Event::Stop); }
...
pub unsafe fn memory(_:JNIEnv, _:JClass, _:JObject) { wake(Event::LowMemory); }
...
// }
//
// unsafe extern "C" fn on_content_rect_changed(activity: *mut ANativeActivity,
// rect: *const ARect) { let rect = Rect {
// left: (*rect).left as _,
// top: (*rect).top as _,
