			...
			match self.first_event.take() {
				Some(EventSource::Callback) => {
					match ndk_glue::poll_events().unwrap() {
						...
						Event::ConfigChanged => {
							// #[allow(deprecated)] // TODO: use ndk-context
							// instead
							// let am = ndk_glue::native_activity().
							// asset_manager(); let config =
							// Configuration::from_asset_manager(&am);
							...
							// f64::EPSILON {   let mut size =
							// MonitorHandle.size();   let event =
							// event::Event::WindowEvent {     window_id:
							// window::WindowId(WindowId),     event:
							// event::WindowEvent::ScaleFactorChanged {
							//       new_inner_size: &mut size,
							//       scale_factor,
							//     },
							//   };
							//   call_event_handler!(event_handler,
							// self.window_target(), control_flow, event);
							// }
						},
...
fn keycode_to_logical(keycode:ndk::event::Keycode, native:NativeKeyCode) -> Key<'static> {
	...
	match keycode {
		...
		//-------------------------------------------------------------------------------
		// Reporting unidentified, because the specific character is layout
		// dependent. (I'm not sure though)
...
pub fn keycode_to_scancode(_code:KeyCode) -> Option<u32> { None }
