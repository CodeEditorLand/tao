...
pub type XErrorHandler =
	Option<unsafe extern fn(*mut ffi::Display, *mut ffi::XErrorEvent) -> libc::c_int>;
...
impl XConnection {
	...
	/// Checks whether an error has been triggered by the previous function
	/// calls.
	#[inline]
	...
	#[inline]
	pub fn ignore_error(&self) { *self.latest_error.lock() = None; }
}
...
/// Error returned if this system doesn't have XLib or can't create an X
/// connection.
#[derive(Clone, Debug)]
...
impl From<ffi::OpenError> for XNotSupported {
	#[inline]
	fn from(err:ffi::OpenError) -> XNotSupported { XNotSupported::LibraryOpenError(err) }
}
