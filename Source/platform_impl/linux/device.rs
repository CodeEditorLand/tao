...
pub fn spawn(device_tx:glib::Sender<DeviceEvent>) {
	std::thread::spawn(move || unsafe {
		...
		loop {
			...
			// Specifically, this involves all of the KeyPress events in compose/pre-edit
			// sequences, along with an extra copy of the KeyRelease events. This also
			// prevents backspace and arrow keys from being detected twice.
			if xlib::True == {
