...
/// Spawn Device event thread. Only works on x11 since wayland doesn't have such
/// global events.
pub fn spawn(device_tx:glib::Sender<DeviceEvent>) {
	std::thread::spawn(move || unsafe {
		...
		loop {
			...
			// XFilterEvent tells us when an event has been discarded by the
			// input method. Specifically, this involves all of the KeyPress
			// events in compose/pre-edit sequences, along with an extra copy
			// of the KeyRelease events. This also prevents backspace and
