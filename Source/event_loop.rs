...
impl<T> fmt::Debug for EventLoop<T> {
	fn fmt(&self, f:&mut fmt::Formatter<'_>) -> fmt::Result { f.pad("EventLoop { .. }") }
}
...
impl EventLoopBuilder<()> {
	...
	#[inline]
	pub fn new() -> Self { Self::with_user_event() }
}
impl<T> EventLoopBuilder<T> {
	...
	/// to platforms that require it. `EventLoopBuilderExt::any_thread`
...
impl EventLoop<()> {
	...
	#[inline]
	pub fn new() -> EventLoop<()> { EventLoopBuilder::new().build() }
}
...
impl<T> EventLoop<T> {
	/// Hijacks the calling thread and initializes the tao event loop with the
...
impl<T> Deref for EventLoop<T> {
	...
	fn deref(&self) -> &EventLoopWindowTarget<T> { self.event_loop.window_target() }
}
...
impl<T> EventLoopWindowTarget<T> {
	...
	#[inline]
	pub fn primary_monitor(&self) -> Option<MonitorHandle> { self.p.primary_monitor() }
	...
	///
	/// Since the [`DeviceEvent`] capture can lead to high CPU usage for
	/// unfocused windows, tao will ignore them by default for unfocused
	/// windows. This method allows changing this filter at runtime to
	/// explicitly capture them again.
	///
	/// ## Platform-specific
	///
	...
	/// ## Platform-specific
	///
	/// - **Windows:** Unsupported. Use the Progress Bar Function Available in
	///   Window (Windows can have different progress bars for different window)
...
unsafe impl<T> rwh_05::HasRawDisplayHandle for EventLoopWindowTarget<T> {
	fn raw_display_handle(&self) -> rwh_05::RawDisplayHandle { self.p.raw_display_handle_rwh_05() }
}
...
impl<T:'static> fmt::Debug for EventLoopProxy<T> {
	fn fmt(&self, f:&mut fmt::Formatter<'_>) -> fmt::Result { f.pad("EventLoopProxy { .. }") }
}
