...
pub enum Event<'a, T:'static> {
	...
	///
	/// During each iteration of the event loop, Tao will aggregate duplicate
	...
	/// Emitted when the app is open by external resources, like opening a file
	/// or deeplink.
	Opened { urls:Vec<url::Url> },

	/// ## Platform-specific
	///
	/// - **macOS**: https://developer.apple.com/documentation/appkit/nsapplicationdelegate/1428638-applicationshouldhandlereopen
	///   with return value same as hasVisibleWindows
	/// - **Other**: Unsupported.
	#[non_exhaustive]
...
pub enum WindowEvent<'a> {
	...
	/// ## Platform-specific
	///
	/// - **Linux(Wayland)**: will always be (0, 0) since Wayland doesn't
	///   support a global cordinate system.
	...
	/// ## Platform-specific
	///
	/// - **Windows / Linux:** Only fired if the [`crate::window::Window`] is
	///   dropped.
	/// - **macOS:** Fired if the [`crate::window::Window`] is dropped or the
	///   dock `Quit` item is clicked.
	Destroyed,
...
impl<'a> WindowEvent<'a> {
	pub fn to_static(self) -> Option<WindowEvent<'static>> {
		...
		match self {
			...
			KeyboardInput { device_id, event, is_synthetic } => {
				Some(KeyboardInput { device_id, event, is_synthetic })
			},
...
pub enum DeviceEvent {
	...
	/// arbitrary input devices that tao supports on this platform, including
...
pub struct KeyEvent {
	...
	/// might be the W, A, S, and D keys on a US layout. The position of these
	/// keys is more important than their label, so they should map to Z, Q,
	/// S, and D on an "AZERTY" layout. (This value is `KeyCode::KeyW` for the
	/// Z key on an AZERTY layout.)
	///
	/// Note that `Fn` and `FnLock` key events are not guaranteed to be emitted
	/// by `tao`. These keys are usually handled at the hardware or OS level.
	pub physical_key:keyboard::KeyCode,
