...
use crate::{
	...
	platform_impl::{Parent, get_aux_state_mut, set_badge_label},
...
pub trait WindowExtMacOS {
	...
	/// A Boolean value that indicates whether the title bar draws its
	/// background.
	///
...
impl WindowExtMacOS for Window {
	...
	#[inline]
	fn tabbing_identifier(&self) -> String { self.window.tabbing_identifier() }
	...
	#[inline]
	fn set_badge_label(&self, label:Option<String>) { self.window.set_badge_label(label); }
}
...
pub trait EventLoopExtMacOS {
	...
	/// Used to prevent the application from automatically activating when
	/// launched if another application is already active
	///
	/// The default behavior is to ignore other applications and activate when
	/// launched.
	///
	/// This function only takes effect if it's called before calling
	/// [`run`](crate::event_loop::EventLoop::run) or
