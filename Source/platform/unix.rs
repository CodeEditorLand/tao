...
pub trait WindowExtUnix {
	/// Create a new Tao window from an existing GTK window. Generally you
	/// should use the non-Linux `WindowBuilder`, this is for those who need
	/// lower level window access and know what they're doing.
	fn new_from_gtk_window<T:'static>(
		event_loop_window_target:&EventLoopWindowTarget<T>,
		window:gtk::ApplicationWindow,
	) -> Result<Window, OsError>;

	/// Returns the `gtk::ApplicatonWindow` from gtk crate that is used by this
	/// window.
	fn gtk_window(&self) -> &gtk::ApplicationWindow;

	/// Returns the vertical `gtk::Box` that is added by default as the sole
	/// child of this window. Returns `None` if the default vertical `gtk::Box`
...
pub trait WindowBuilderExtUnix {
	...
	///
	/// When tranparent attribute is enabled, we will call `connect_draw` and
	/// draw a transparent background. For anyone who wants to draw the
	/// background themselves, set this to `false`. Default is `true`.
	fn with_transparent_draw(self, draw:bool) -> WindowBuilder;

	/// Whether to enable or disable the double buffered rendering of the
	/// window.
	///
	/// Default is `true`.
	...
	/// [`WindowAttributes::transparent`](crate::window::WindowAttributes::transparent)
	/// is `true`
	...
	/// Whether to set cursor moved event. Cursor event is suited for native GUI
	/// frameworks and games. But it can block gtk's own pipeline occasionally.
	/// Turn this off can help Gtk looks smoother.
	///
	/// Default is `true`.
	fn with_cursor_moved_event(self, cursor_moved:bool) -> WindowBuilder;

	/// Whether to create a vertical `gtk::Box` and add it as the sole child of
	/// this window. Created by default.
...
pub trait EventLoopWindowTargetExtUnix {
	...
	// /// Returns a pointer to the `wl_display` object of wayland that is used
	// by this /// `EventLoopWindowTarget`.
	// ///
	// /// Returns `None` if the `EventLoop` doesn't use wayland (if it uses
	// xlib for example). ///
	// /// The pointer will become invalid when the winit `EventLoop` is
	// destroyed. fn wayland_display(&self) -> Option<*mut raw::c_void>;
...
impl<T> EventLoopWindowTargetExtUnix for EventLoopWindowTarget<T> {
	#[inline]
	fn is_wayland(&self) -> bool { self.p.is_wayland() }

	#[inline]
	fn is_x11(&self) -> bool { !self.p.is_wayland() }
	...
	#[inline]
	fn gtk_app(&self) -> &gtk::Application { &self.p.app }
}

unsafe extern fn x_error_callback(
