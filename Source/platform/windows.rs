...
pub trait EventLoopBuilderExtWindows {
	...
	/// A callback to be executed before dispatching a win32 message to the
	/// window procedure. Return true to disable tao's internal message
	/// dispatching.
	///
	/// # Example
	///
	...
	/// so it is recommended to always use the same theme used for this app-wide
	/// setting or use `None` so it automatically uses the theme of this method
...
pub trait WindowExtWindows {
	...
	///
	/// Enabling the shadow causes a thin 1px line to appear on the top of the
	/// window.
	fn set_undecorated_shadow(&self, shadow:bool);

	/// Sets right-to-left layout.
	///
	/// Enabling this mainly flips the orientation of menus and title bar
	/// buttons
...
impl WindowExtWindows for Window {
	#[inline]
	fn hinstance(&self) -> isize { self.window.hinstance().0 as _ }

	#[inline]
	fn hwnd(&self) -> isize { self.window.hwnd().0 as _ }
	...
	#[inline]
	fn reset_dead_keys(&self) { self.window.reset_dead_keys(); }
	...
	#[inline]
	fn set_overlay_icon(&self, icon:Option<&Icon>) { self.window.set_overlay_icon(icon); }
}
...
pub trait WindowBuilderExtWindows {
	...
	/// `COINIT_APARTMENTTHREADED`) on the same thread. Note that tao may still
	/// attempt to initialize COM API regardless of this option. Currently only
	/// fullscreen mode does that, but there may be more in the future.
	/// If you need COM API with `COINIT_MULTITHREADED` you must initialize it
	/// before calling any tao functions. See <https://docs.microsoft.com/en-us/windows/win32/api/objbase/nf-objbase-coinitialize#remarks> for more information.
...
impl MonitorHandleExtWindows for MonitorHandle {
	...
	#[inline]
	fn hmonitor(&self) -> isize { self.inner.hmonitor().0 as _ }
}
