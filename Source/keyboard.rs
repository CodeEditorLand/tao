...
impl ModifiersState {
	...
	pub fn shift_key(&self) -> bool { self.intersects(Self::SHIFT) }

	/// Returns `true` if the control key is pressed.
	pub fn control_key(&self) -> bool { self.intersects(Self::CONTROL) }

	/// Returns `true` if the alt key is pressed.
	pub fn alt_key(&self) -> bool { self.intersects(Self::ALT) }

	/// Returns `true` if the super key is pressed.
	pub fn super_key(&self) -> bool { self.intersects(Self::SUPER) }
}
...
pub enum KeyCode {
	...
	/// have a <kbd>Clear</kbd> key that is separate from the
	/// <kbd>NumLock</kbd> key. On the Mac, the numpad <kbd>Clear</kbd> key is
	/// encoded as [`NumLock`].
	///
	...
	/// separator is a "." (e.g., Brazil), this key may generate a
	/// <kbd>.</kbd>.
	NumpadComma,
...
pub enum Key<'a> {
	...
	/// concurrent or subsequent keyboard input. This key value is also used
	/// for the Apple <kbd>Option</kbd> key.
	Alt,
	...
	/// subsequent keyboard input. This key value is used for the "Windows
	/// Logo" key and the Apple `Command` or `⌘` key.
	///
	...
	/// sequence, but is now more generally used to exit or "escape" the
	/// current context, such as closing a dialog or exiting full screen mode.
	Escape,
	Execute,
	...
	/// playing at normal speed. (`APPCOMMAND_MEDIA_PLAY`,
	/// `KEYCODE_MEDIA_PLAY`)
	MediaPlay,
	...
	/// with launching "My Computer", and may have a computer symbol on the
	/// key. (`APPCOMMAND_LAUNCH_APP1`)
	LaunchApplication1,
...
impl<'a> From<&'a str> for Key<'a> {
	fn from(src:&'a str) -> Key<'a> { Key::Character(src) }
}
